...
export default function EpisodeCardGenerator() {
  ...
  return (
    <div className="min-h-screen magi-grid relative crt-flicker overflow-hidden" role="application" aria-label="Evangelion Title Card Generator">
      {/* SEO Content */}
      <h1 className="sr-only">Evangelion Title Card Generator - Create authentic Neon Genesis Evangelion episode cards</h1>
      <p className="sr-only">Generate custom Evangelion episode title cards with authentic typography.</p>
      ...
      <header className="relative z-30 bg-magi-bg-panel border-b border-magi-grid" role="banner">
        <div className="h-12 flex items-center px-3 md:px-4">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span className="text-base md:text-lg font-bold text-magi-cyan tracking-widest truncate">MAGI SYSTEM</span>
          </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-magi-bg/95 pt-12">
          <div className="p-4 space-y-4">
            ...
            <button onClick={() => setIsMobileMenuOpen(false)} className="w-full py-3 border border-magi-cyan text-magi-cyan">CLOSE</button>
          </div>
        </div>
      )}
      ...
      <main className="relative z-10 h-[calc(100vh-48px)] flex flex-col">
            ...
            >
              <div className="bg-magi-bg-panel border-b border-magi-grid px-3 py-1.5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-xs">
                  <button onClick={() => setCanvasScale(Math.max(0.2, canvasScale - 0.05))} className="text-magi-text-dim">−</button>
                  <span className="text-magi-cyan w-10 text-center">{Math.round(canvasScale * 100)}%</span>
                  <button onClick={() => setCanvasScale(Math.min(1.0, canvasScale + 0.05))} className="text-magi-text-dim">+</button>
                </div>
              </div>
              <EpisodeCard
                ref={cardRef}
        ...
        <section className="h-auto max-h-[45vh] md:h-80 bg-magi-bg-panel border-t border-magi-grid flex flex-col md:flex-row" aria-label="Editor">
          <nav className="w-full md:w-48 border-b md:border-r border-magi-grid flex md:flex-col overflow-x-auto" role="tablist">
            <button
            ...
            {activeTab === 'content' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={topText} onChange={(e) => setTopText(e.target.value)} className="w-full bg-magi-bg border border-magi-grid text-magi-text px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="text-magi-text-dim text-xs">Second Line</label>
                  <input type="text" value={midText} onChange={(e) => setMidText(e.target.value)} className="w-full bg-magi-bg border border-magi-grid text-magi-text px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="text-magi-text-dim text-xs">Third Line</label>
                  <input type="text" value={botText} onChange={(e) => setBotText(e.target.value)} className="w-full bg-magi-bg border border-magi-grid text-magi-text px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="text-magi-text-dim text-xs">Episode</label>
                  <input type="text" value={epText} onChange={(e) => setEpText(e.target.value)} className="w-full bg-magi-bg border border-magi-grid text-magi-text px-3 py-2 text-sm" />
                </div>
                <div className="lg:col-span-2">
                  <label className="text-magi-text-dim text-xs">Title</label>
                  <textarea value={titleText} onChange={(e) => setTitleText(e.target.value)} rows={3} className="w-full bg-magi-bg border border-magi-grid text-magi-text px-3 py-2 text-sm" />
                </div>
              </div>
            )}
            ...
            {activeTab === 'style' && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <button onClick={() => setTitleStyle('serif')} className={`w-full p-2 border ${titleStyle === 'serif' ? 'border-magi-cyan text-magi-cyan' : 'border-magi-grid'}`}>Serif</button>
                  <button onClick={() => setTitleStyle('sans')} className={`w-full p-2 border mt-2 ${titleStyle === 'sans' ? 'border-magi-cyan text-magi-cyan' : 'border-magi-grid'}`}>Sans</button>
                </div>
                <div>
                  <label className="text-magi-text-dim text-xs block mb-2">Align</label>
                  <div className="flex gap-2">
                    {(['left', 'center', 'right'] as const).map(align => (
                      <button key={align} onClick={() => setTitleAlign(align)} className={`flex-1 p-2 border ${titleAlign === align ? 'border-magi-cyan text-magi-cyan' : 'border-magi-grid'}`}>{align[0].toUpperCase()}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="text-magi-text-dim text-xs block mb-2">Ratio</label>
                  <button onClick={() => setAspectRatio('standard')} className={`w-full p-2 border ${aspectRatio === 'standard' ? 'border-magi-cyan text-magi-cyan' : 'border-magi-grid'}`}>900×675</button>
                  <button onClick={() => setAspectRatio('wide')} className={`w-full p-2 border mt-2 ${aspectRatio === 'wide' ? 'border-magi-cyan text-magi-cyan' : 'border-magi-grid'}`}>1280×720</button>
                </div>
              </div>
            )}
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-5 rounded-full p-0.5 ${effects.glowEnabled ? 'bg-magi-cyan' : 'bg-magi-grid'}`}>
                      <button onClick={() => updateEffect('glowEnabled', !effects.glowEnabled)} className={`w-4 h-4 bg-white rounded-full transition-transform ${effects.glowEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        {['#ffffff', '#00d4aa', '#ffb000', '#00ff41', '#ff0040'].map(color => (
                          <button key={color} onClick={() => updateEffect('glowColor', color)} className={`w-8 h-8 rounded border-2 ${effects.glowColor === color ? 'border-white' : 'border-transparent'}`} style={{ backgroundColor: color }} />
                        ))}
                      </div>
                    </div>
                    <div>
                      <label className="text-magi-text-dim text-xs">Blur: {effects.glowBlur}px</label>
                      <input type="range" min="5" max="50" value={effects.glowBlur} onChange={(e) => updateEffect('glowBlur', parseInt(e.target.value))} className="w-full" />
                    </div>
                  </div>
                )}
              </div>
